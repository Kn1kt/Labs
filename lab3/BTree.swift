...
// MARK: BTreeNode extesnion: BinarySearch
extension BTreeNode {
  
  /**
   *  Returns the index for a given `key`.
   *  If the `key` is not found, returns index of other `key` maximum close to the given
   *
   *  - Parameters:
   *    - a: input array of keys
   *    - key: the key that need searching
   *    - range: range of available indexes in array
   *
   */
  private func binarySearch(_ a: [Key], for key: Key, range: Range<Int>? = nil) -> Int {
    let range = range ?? a.startIndex ..< a.endIndex
    
    if range.lowerBound == range.upperBound {
      return range.upperBound
      
    } else {
      let midIndex = range.startIndex + (range.upperBound - range.lowerBound) / 2
      
      if a[midIndex] > key {
        return binarySearch(a, for: key, range: range.lowerBound ..< midIndex)
        
      } else if a[midIndex] < key {
        return binarySearch(a, for: key, range: midIndex + 1 ..< range.upperBound)
...
extension BTreeNode {
  ...
  func value(for key: Key) -> Value? {
    ...
    let rawIndex = binarySearch(keys, for: key)
    
    if rawIndex < keys.endIndex && keys[rawIndex] == key {
      return values[rawIndex]
    }
    
    let index = (rawIndex - 1) < 0 ? rawIndex : rawIndex - 1
...
extension BTreeNode {
  ...
  func insert(_ value: Value, for key: Key) {
    ...
    let index = binarySearch(keys, for: key)
  ...
  private func split(child: BTreeNode, atIndex index: Int) {
    ...
    let key = child.keys.remove(at: middleIndex)
    let value = child.values.remove(at: middleIndex)
    keys.insert(key, at: index)
    values.insert(value, at: index)
    
    let rightSiblingKeysIndices = child.keys.indices.suffix(from: middleIndex)
    let rightSiblingValuesIndices = child.values.indices.suffix(from: middleIndex)
    let rightSibling = BTreeNode(owner: owner,
                                 keys: Array(child.keys[rightSiblingKeysIndices]),
                                 values: Array(child.values[rightSiblingValuesIndices])
    )
    child.keys.removeSubrange(rightSiblingKeysIndices)
    child.values.removeSubrange(rightSiblingValuesIndices)
    
    children!.insert(rightSibling, at: (index + 1))
    
    if child.children != nil {
      let childIndices = child.children!.indices.suffix(from: (middleIndex + 1))
      rightSibling.children = Array(
        child.children![childIndices]
      )
      child.children!.removeSubrange(childIndices)
    }
  }
}
...
extension BTreeNode {
  ...
  func remove(_ key: Key) {
    ...
    if rawIndex < keys.endIndex && keys[rawIndex] == key {
      if isLeaf {
        keys.remove(at: rawIndex)
        values.remove(at: rawIndex)
        owner.numberOfKeys -= 1
      } else {
        let predecessor = children![rawIndex].inorderPredecessor
        keys[rawIndex] = predecessor.keys.last!
        values[rawIndex] = predecessor.values.last!
        children![rawIndex].remove(keys[rawIndex])
        if children![rawIndex].numberOfKeys < owner.order {
          fix(childWithTooFewKeys: children![rawIndex], atIndex: rawIndex)
        }
      }
    } else if key < keys[index] {
   ...
   */
  private func move(keyAtIndex index: Int,
                    to targetNode: BTreeNode,
                    from node: BTreeNode,
                    at position: BTreeNodePosition) {
    switch position {
    case .left:
      ...
      keys[index] = node.keys.removeLast()
      values[index] = node.values.removeLast()
    ...
    case .right:
      ...
      keys[index] = node.keys.removeFirst()
      values[index] = node.values.removeFirst()
  ...
  private func merge(child: BTreeNode, atIndex index: Int, to position: BTreeNodePosition) {
    ...
    case .left:
      ...
      children![(index - 1)].keys += [keys[(index - 1)]] + child.keys
      
      children![(index - 1)].values += [values[(index - 1)]] + child.values
...
extension BTree {
  ...
  private func splitRoot() {
    ...
    let newRoot = BTreeNode<Key, Value>(owner: self,
...
extension BTree {
  ...
  public func removeAll() {
    rootNode.keys.removeAll()
    rootNode.values.removeAll()
...
// MARK: BTree extension: Iterator

extension BTree {
  
  public func makeIterator() -> BTreeIterator<Key, Value>? {
    return BTreeIterator(tree: self)
  }
}
